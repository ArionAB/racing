...
| `custom_ice_field_ranges` | campuri de placi crapate (Baikal): interval `(frac_start, frac_end)` pe care culoarul de gheata se sparge in placi Voronoi ridicate si DENIVELATE (trepte de 8-15 cm intre placi — se simt in suspensie la orice viteza), cu 8 placi vii pe culoar care se asaza sub roti cu trosnet, se inclina si acumuleaza stricaciune la fiecare trecere — a 3-a trecere normala / a 2-a cu turbo le rupe (repunere pentru cine e pe ele; revin in ~2 s); doar pe `custom_ice_ranges`. Sonda: `ProbeIceField` |
...
| `custom_viaduct_ravines` | care dintre `custom_ravines` sunt VIADUCTE (indici): golul e si SUB sosea, tablierul ramane in aer pe fusta lui, umerii de pietris se sting; dedesubt pui pilele/arcadele din kit sub `DecorManual` (Baikal: `viaduct_pier.glb` / `viaduct_arch.glb` / `viaduct_end.glb` — cate un fisier pe modul, deci nimic de stins la instantiere — pe axa, baza la cota drumului − 12.7 / − 13.9 la pile). Se combina cu `custom_cornice_ravines` ca sa cazi de pe margine. Verifici din lateral cu `Snapshot --eye=x,y,z --look=x,y,z` |
...
3. In Inspector, `kind`: bariera mobila, bolovan, morisca, tren, tromba,
   ...
   altfel se sare, cu avertisment in Output), camp de placi crapate (120 m
   centrati pe nod, placi vii care se inclina si se rup — are sens doar pe o
   portiune din `custom_ice_ranges`; si prin `custom_ice_field_ranges`, ca
   interval), tren pe sens (sina in lungul soselei, vine din
