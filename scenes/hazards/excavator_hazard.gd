...
func _ready() -> void:
	...
	body_box.size = Vector3(2.6, 2.8, 2.6)
	body_shape.shape = body_box
	body_shape.position = Vector3(0, 1.4, 0.5)
	add_child(body_shape)
	# Bratul intins peste drum (solid doar cand e coborat).
	_arm_shape = CollisionShape3D.new()
	var arm_box := BoxShape3D.new()
	arm_box.size = Vector3(1.4, 1.8, 5.4)
	_arm_shape.shape = arm_box
	_arm_shape.position = Vector3(0, 1.0, -3.6)
