...
def _profile(kind, height, r_base, r_neck, seed, steps=7):
    ...
    # BUZE DE STRAT (masuratoarea lui POI E, portata pe familiile de silueta).
    # Taperul neted cu jitter mic e INVIZIBIL la 40 m: orice perturbatie sub
    # ~10% din raza dispare din silueta. Un strat de tuf se citeste doar daca
    # arunca propria linie de umbra, adica daca are o fata de sus ORIZONTALA
    # care iese in consola peste stratul de deasupra.
    #
    # Se face din DOUA inele la ACEEASI cota: raza larga, apoi raza ingusta.
    # `revolve` interpoleaza intre inele, deci fara perechea la aceeasi cota
    # treapta redevine o tesitura — si o consola tesita citeste tot ca taper.
    # Adancimea 0.30 m e cifra din verdictele lui E; nu se tesesc.
    #
    # Diferenta fata de varianta lui E: acolo buzele CONSTRUIAU profilul (cinci
    # segmente stivuite uniform), deci stergeau familia de silueta. Aici se
    # ADAUGA peste profilul familiei, care ramane cel care da forma. De aia nu
    # se scade nimic din raza de sub buza: profilul ajunge deja unde trebuie.
    LIP = 0.30
    lips = []
    for u in (0.34, 0.52, 0.70):
        z = height * u
        r = _r_at(prof, z)
        if r - LIP < r_neck * 1.02:
            continue          # prea sus/prea subtire: buza ar taia gatul
        lips.append((r, z))
        lips.append((max(r - LIP, r_neck), z))
    if lips:
        prof = sorted(prof + lips, key=lambda rz: rz[1])
    return prof
