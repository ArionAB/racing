extends Node
## Verifica: (1) turnurile din randul apropiat nu intra in carosabil, (2) baza e
## sub teren (nu pluteste), (3) unghiul de depresie spre varf de la camera.
func _ready() -> void:
	...
	for ch in root.get_children():
		if not str(ch.name).begins_with("turn_sub_piata"): continue
		var n := ch as Node3D
		var ab := _aabb(n)
